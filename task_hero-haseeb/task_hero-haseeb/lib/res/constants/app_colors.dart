import 'package:flutter/material.dart';

class AppColors {
  static const Color primary = Color(0xFF6A1B9A);
  static const Color purple = Color(0xFF6A1B9A); // Updated to purple shade 900
  static const Color yellowLight = Color(0xFFF6CE42);
  
  static const Color lightGray = Color.fromARGB(255, 241, 241, 241); // Added light gray color

  static const Color textGrey = Color(0xFF434343);

  static const Color white = Color(0xFFffffff);
  static const Color black = Color(0xFF000000);
  static const Color red = Color(0xFFFF0000);
  static const Color gry = Color(0xFF8C93A3);
  static const Color lightBlack = Color(0xff161616);
  static const Color aqua = Color(0xff26DBDC);
  static const Color deepSkyBlue = Color(0xff1787E7);
  static const Color black1 = Color(0xff212121);
  static const Color p1 = Color(0xFF6A1B9A);

  static const Color orange = Color(0xffFF981F);
  static const Color yellow = Color(0xFFFACC15);
}
